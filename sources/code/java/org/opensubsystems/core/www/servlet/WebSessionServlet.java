...
public class WebSessionServlet extends HttpServlet
{
      ...
      try
      {
         ...
         try
         {
            ...
            if (hsrqRequest.isSecure() != shouldRequestBeSecure())
            {
               ...
               s_logger.log(Level.FINER, "Redirecting due to HTTP(S) from {0} to {1}", 
                            new Object[]{strOriginal, stsRedirect});
            ...
            else
            {
               ...
               {
                  s_logger.log(Level.FINEST, "Session {0} is still new and"
                               + " hanshake is required", hsSession.getId());
            ...
            OSSException
   {
      ...
      if ((strValidatorClass != null) && (strValidatorClass.length() > 0))
      {
         m_sessionValidator = (SessionValidator)ClassUtils.createNewInstance(
                                                   strValidatorClass);
         s_logger.log(Level.FINEST, "Instantiated session validator {0}", 
                      strValidatorClass);        
      }
   ...
   )
   {
      ...
      String strLoginRedirect;
      
      strLoginRedirect = (String)hsSession.getAttribute(LOGIN_FORWARD_SESSION_PARAM);
      s_logger.log(Level.FINEST, "Retrieved login redirect to {0}", 
                   strLoginRedirect);
   ...
   )
   {
      ...
      if (hsSession.getAttribute(LOGIN_FORWARD_SESSION_PARAM) == null)
      {
         ...
         s_logger.log(Level.FINEST, "Saved login redirect {0}", strFullRedirectURL);
         bReturn = true;
      }
            ...
            IOException
   {
      // We cannot use RequestDispatcher because browser would cache this
      // login page as the requested page
      s_logger.log(Level.FINEST, "Redirecting to login {0}{1}", 
                   new Object[]{hsrqRequest.getContextPath(), m_strLoginURL});
            ...
            IOException
   {
      // We cannot use RequestDispatcher because browser would cache this
      // login page as the requested page
      s_logger.log(Level.FINEST, "Redirecting to handshake {0}{1}", 
                   new Object[]{hsrqRequest.getContextPath(), m_strLoginURL});
